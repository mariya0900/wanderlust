...
class _TripDestinationMapState extends State<TripDestinationMap> {
  ...
  Widget build(BuildContext context){
    ...
    widget.locations=widget.itinerary.map((e) => LatLng(
      double.parse(e.location.split('-')[1].split(':')[0]), double.parse(e.location.split(',')[1].split(':')[1]) )).toList();

    //mapToCoordinates(widget.itinerary);
    print(widget.locations);// prints empty list
    LatLng center=findCenter(widget.locations);
    return Scaffold(appBar: AppBar(title: Text("Destination Map"),),
    body: FlutterMap(
      options: MapOptions(zoom: 7.0, bounds: LatLngBounds(findMin(widget.locations), findMax(widget.locations)) , center: center),
      layers: [
        TileLayerOptions(
            urlTemplate:
        ...
        MarkerLayerOptions(
          markers: widget.locations.map((e) => Marker(
              point: e, 
        ...
        PolylineLayerOptions(
            polylines: [
              Polyline(color: Colors.blueAccent, strokeWidth: 2.0, points: 
                widget.locations.map((e) =>(e)).toList(),
